...
arcagent automatically optimizes API costs through prompt caching. For providers that support it, stable content (system prompts, tool definitions, conversation history) is cached between turns, giving you up to **90% savings** on input tokens.
...
arcagent places up to 3 cache breakpoints automatically:
...
DeepSeek's API manages context caching automatically. arcagent does not send
...
```rust
use arcagent::{CacheConfig, CacheStrategy};
...
This disables arcagent-managed cache hints for providers such as Anthropic. It
