...
```rust
use arcagent::shared_state::FileBackend;
...
```rust
use arcagent::shared_state::{SharedStateBackend, SharedStateError};
...
Sub-agents can use any provider supported by arcagent — not just Anthropic. Pass a `ModelConfig` to configure the base URL, compat flags, and other provider-specific settings:
